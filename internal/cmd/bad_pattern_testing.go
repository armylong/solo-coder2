...
// Bad Pattern评测入口，按action分发
func (d *badPatternTestingCmd) BadPatternTestingHandler(c *cli.Context) error {
	ctx := c.Context
	action := ""
	if c.NArg() > 0 {
		action = c.Args().Get(0)
...
func (d *badPatternTestingCmd) formatWorks(ctx context.Context, workSpace string) {
	...
	for _, entry := range entries {
		...
		workInitData = re.ReplaceAll(workInitData, []byte(""))
